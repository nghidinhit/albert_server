...
def main():
    ...
    logger.info(f"  Total num examples = {total_train_examples}")
    ...
    for epoch in range(args.epochs):
        for idx in range(args.file_num):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if global_step % int(args.num_eval_steps) == 0:
                    now = time.time()
                    eta = now - start_time
                    if eta > 3600:
                ...
                print('\rEpoch: %d - step: %d - loss: %f - %fs/it' % (epoch, step, loss.item(), time.time() - time_1), end='\r')

                # if global_step % int(args.num_save_steps) == 0:
